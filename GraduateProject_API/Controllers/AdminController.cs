...
public class AdminController : ControllerBase
{
    ...
    {
        var appointments = await adminRepository.GetAllAppointmentsAsync();
        return Ok(appointments);
    }
    [HttpGet("GetAllReportFiles")]
    public async Task<IActionResult> GetAllReportFiles()
    {
        var reports = await adminRepository.GetAllReportFilesAsync();
        return Ok(reports);
    }
    ...
    public async Task<IActionResult> UpdateUser(int userId, [FromBody] AdminUpdateUserDTO dto)
    {
        ...
        var result = await adminRepository.UpdateUserAsync(userId, dto);
        if (!result)
    ...
    public async Task<IActionResult> CreateUser([FromBody] AdminCreateUserDTO dto)
    {
        ...
        if (dto.Role == "Doctor")
        {
            ...
            {
                UserId = user.Id,
                Specialization = dto.Specialization, // ✅ حفظ التخصص في جدول Doctors
                PerformanceScore = 0,
                Workload = 0
            });
        }
    ...
    [HttpGet("GetDoctorHistory/{userId}")]
    public async Task<IActionResult> GetDoctorHistory(int userId)
    {
        var doctor = await _context.Doctors
            .Include(d => d.User)
            .Where(d => d.UserId == userId)
            .SelectMany(d => _context.Appointments
                .Where(a => a.DoctorID == d.DoctorID)
                .Select(a => new
                {
                    Date = a.DateTime,
                    Specialty = d.User.Specialization,
                    Notes = a.Notes
                }))
            .ToListAsync();
    ...
    [HttpGet("GetPatientHistory/{userId}")]
    public async Task<IActionResult> GetPatientHistory(int userId)
    {
        var patient = await _context.Patients
            .Include(p => p.User)
            .Where(p => p.UserId == userId)
            .SelectMany(p => _context.MedicalHistories
